...
# Set up logging
log_file = "resource_tracking.log"
...
def busco_lineage_exists(download_path, lineage):
    return download_path and os.path.exists(os.path.join(download_path, "lineages", lineage))

if download_path and busco_lineage_exists(download_path, args.busco_lineage):
    logging.info(f"BUSCO lineage '{args.busco_lineage}' found in '{download_path}'. Skipping download.")
else:
...
def convergence_callback(study, trial):
    has_converged, converged_methods = convergence_detector.update(study.best_value, trial.number)
    ...
    if has_converged:
        ...
        logging.info(f"Majority convergence not yet met. Partial convergence detected by: {methods}")
...
try:
    ...
    if not args.force_rerun:
        ...
        if len(study.trials) > 0:
            logging.info(f"Study has {len(study.trials)} trials. Checking convergence status...")
        ...
        if trial.number == 0:
            ...
            if args.sensitive:
                final_command = build_hifiasm_command(
                    prefix=base_name,
                    x=best_x, y=best_y, s=best_s, n=best_n, m=best_m, p=best_p,
                    haploid_genome_size=KNOWN_GENOME_SIZE,
                    threads=threads,
                    sensitive=True,
                    D=trial.params['D'],
            ...
            if Path(fasta_file).exists():
                logging.info(f"Final assembly FASTA {fasta_file} already exists. Skipping final assembly step.")
            else:

                logging.info("Running final assembly with best parameters:\n%s", final_command)
                final_hifiasm_log = logs_dir / "final_hifiasm.log"
                with open(final_hifiasm_log, "w") as logf:
                    subprocess.run(final_command, shell=True, stdout=logf, stderr=subprocess.STDOUT)
...
vis.plot_optimization_history(study).write_html(optuna_dir / "optuna_optimization_history.html")
vis.plot_param_importances(study).write_html(optuna_dir / "optuna_param_importance.html")
vis.plot_parallel_coordinate(study).write_html(optuna_dir / "optuna_parallel_coordinates.html")
...
for trial_num in available_trials:
    if trial_num == 0:  # skip the default assembly trial as this does not parameters set
        continue
    for param in param_names:
        try:
            fig = tpe_acquisition_visualizer.plot(study, trial_num, param)
            fig.savefig(os.path.join(output_dir, f"acquisition_trial_{trial_num}_{param}.png"))
            plt.close(fig)
        except Exception as e:
